...
public class ExperimentSet {
	...
	public void run() throws IOException{
		...
		fw.write("ReqTime, quiescenceTime, vcFreenessTime, workWhenFreenessF, workWhenFreenessM, workWhenQuiescenceM, workWhenQuiescenceQ, workWhenRequestF, workWhenRequestM"
				+ ", workWhenRequestQ, lossWorkByQu, lossWorkByVC\n" );
		for (int i=0;i<runNumber; i++){
			params.setSeed(seeds[i]);
			params.reInit();
			Experiment exp= new Experiment(this.graph,this.targetComponent,reqTime);
			exp.run();
			Result res= exp.getResult();

			fw.write(res.reqTime+","+res.quiescenceTime+","+res.vcFreenessTime+","+res.workWhenFreenessF+","+res.workWhenFreenessM+","+res.workWhenQuiescenceM+","+res.workWhenQuiescenceQ+
